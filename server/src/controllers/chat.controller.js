...
export const getPDFChats = async (req, res) => {
  ...
  try {
    ...
    if (!pdf) {
      console.error(
        `[getPDFChats] PDF not found with ID: ${req.params.pdfId} for user: ${userId}`,
      );
    ...
    console.log(
      `[getPDFChats] Successfully retrieved ${chats.length} chats for PDF: ${pdf._id}`,
    );
    res.status(200).json({
      success: true,
      count: chats.length,
...
export const deleteChat = async (req, res) => {
  ...
  try {
    ...
    if (!chat) {
      console.error(
        `[deleteChat] Chat not found with ID: ${req.params.chatId}`,
      );
      return res.status(404).json({
        success: false,
    ...
    if (!pdf) {
      console.error(
        `[deleteChat] User ${req.user._id} not authorized to delete chat ${chat._id}`,
      );
    ...
    // Remove chat from PDF's chats array atomically
    console.log(`[deleteChat] Removing chat reference from PDF: ${pdf._id}`);
    await PDF.findByIdAndUpdate(pdf._id, { $pull: { chats: chat._id } });
